...
#             sample[TEST_FEATS[3]] = features[seq_length*4+3]
            sample[FEATS2[0]] = features[0:1]
            sample[FEATS2[1]] = features[1:2]
            sample[FEATS2[2]] = features[2:3]
            sample[FEATS2[3]] = features[3:4]
            sample[TEST_FEATS2[0]] = features[4:5]
            sample[TEST_FEATS2[1]] = features[5:6]
            sample[TEST_FEATS2[2]] = features[6:7]
            sample[TEST_FEATS2[3]] = features[7:8]
...
#         data[TEST_FEATS[3]] = np.zeros((len(df),), dtype='float32')
        data[FEATS2[0]] = np.zeros(feature_tot_shp, dtype='float32')
        data[FEATS2[1]] = np.zeros(feature_tot_shp, dtype='float32')
        data[FEATS2[2]] = np.zeros(feature_tot_shp, dtype='float32')
        data[FEATS2[3]] = np.zeros(feature_tot_shp, dtype='float32')
        data[TEST_FEATS2[0]] = np.zeros(feature_tot_shp, dtype='float32')
        data[TEST_FEATS2[1]] = np.zeros(feature_tot_shp, dtype='float32')
        data[TEST_FEATS2[2]] = np.zeros(feature_tot_shp, dtype='float32')
        data[TEST_FEATS2[3]] = np.zeros(feature_tot_shp, dtype='float32')
...
#             data[TEST_FEATS[3]][i] = value[TEST_FEATS[3]]
            data[FEATS2[0]][i] = value[FEATS2[0]]
            data[FEATS2[1]][i] = value[FEATS2[1]]
            data[FEATS2[2]][i] = value[FEATS2[2]]
            data[FEATS2[3]][i] = value[FEATS2[3]]
            data[TEST_FEATS2[0]][i] = value[TEST_FEATS2[0]]
            data[TEST_FEATS2[1]][i] = value[TEST_FEATS2[1]]
            data[TEST_FEATS2[2]][i] = value[TEST_FEATS2[2]]
            data[TEST_FEATS2[3]][i] = value[TEST_FEATS2[3]]
            data['ts'][i] = value['t']
            data['ids'][i] = key
...
#         batch_holder[TEST_FEATS[3]] = np.zeros((self._batch_size,), dtype='float32')
        batch_holder[FEATS2[0]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[FEATS2[1]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[FEATS2[2]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[FEATS2[3]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[TEST_FEATS2[0]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[TEST_FEATS2[1]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[TEST_FEATS2[2]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
        batch_holder[TEST_FEATS2[3]] = np.zeros((self._batch_size, self._num_features), dtype='float32')
...
#             batch[TEST_FEATS[3]][i] = self._train[TEST_FEATS[3]][idx]
            batch[FEATS2[0]][i] = self._train[FEATS2[0]][idx]
            batch[FEATS2[1]][i] = self._train[FEATS2[1]][idx]
            batch[FEATS2[2]][i] = self._train[FEATS2[2]][idx]
            batch[FEATS2[3]][i] = self._train[FEATS2[3]][idx]
            batch[TEST_FEATS2[0]][i] = self._train[TEST_FEATS2[0]][idx]
            batch[TEST_FEATS2[1]][i] = self._train[TEST_FEATS2[1]][idx]
            batch[TEST_FEATS2[2]][i] = self._train[TEST_FEATS2[2]][idx]
            batch[TEST_FEATS2[3]][i] = self._train[TEST_FEATS2[3]][idx]
...
#             batch[TEST_FEATS[3]] = batch[TEST_FEATS[3]][:i]
            batch[FEATS2[0]] = batch[FEATS2[0]][:i]
            batch[FEATS2[1]] = batch[FEATS2[1]][:i]
            batch[FEATS2[2]] = batch[FEATS2[2]][:i]
            batch[FEATS2[3]] = batch[FEATS2[3]][:i]
            batch[TEST_FEATS2[0]] = batch[TEST_FEATS2[0]][:i]
            batch[TEST_FEATS2[1]] = batch[TEST_FEATS2[1]][:i]
            batch[TEST_FEATS2[2]] = batch[TEST_FEATS2[2]][:i]
            batch[TEST_FEATS2[3]] = batch[TEST_FEATS2[3]][:i]
            yield batch, i
...
#             batch[TEST_FEATS[3]][i] = self._test[TEST_FEATS[3]][idx]
            batch[FEATS2[0]][i] = self._test[FEATS2[0]][idx]
            batch[FEATS2[1]][i] = self._test[FEATS2[1]][idx]
            batch[FEATS2[2]][i] = self._test[FEATS2[2]][idx]
            batch[FEATS2[3]][i] = self._test[FEATS2[3]][idx]
            batch[TEST_FEATS2[0]][i] = self._test[TEST_FEATS2[0]][idx]
            batch[TEST_FEATS2[1]][i] = self._test[TEST_FEATS2[1]][idx]
            batch[TEST_FEATS2[2]][i] = self._test[TEST_FEATS2[2]][idx]
            batch[TEST_FEATS2[3]][i] = self._test[TEST_FEATS2[3]][idx]
